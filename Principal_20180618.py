    ...
    for ss, info in HIST_SS.iteritems():
        ...
        for data, conteudo in sorted(info.iteritems()):            
            for mudanca in re.findall(r'<td class="HistoryColumn0">Status&nbsp;</td><td class="HistoryColumn1">(.*?)</td>', conteudo):                                
                ...
                HIST_SS_PEN.get(ss).append(Constantes.SG_AGUARDANDO_DES)
            for aguardando_cli in re.findall(r'Added:\n<br />&nbsp;&nbsp;&nbsp;&nbsp;aguardando_urc', conteudo):                                
            #for aguardando_cli in re.findall(r'Added:&#xA;&lt;br />&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;aguardando_urc/cli',conteudo):
                print "ENTROUUUU"
                nova_situacao = getUltimaSituacao(ss)
                HIST_SS_MUD.get(ss).update({data[:-14]:nova_situacao})
                HIST_SS_PEN.get(ss).append(Constantes.SG_AGUARDANDO_CLI)
            for aguardando_des in re.findall(r'Removed:\n<br />&nbsp;&nbsp;&nbsp;&nbsp;aguardando_urc', conteudo):                                
                nova_situacao = getUltimaSituacao(ss)
                HIST_SS_MUD.get(ss).update({data[:-14]:nova_situacao})                                       
                HIST_SS_PEN.get(ss).append(Constantes.SG_AGUARDANDO_DES)
        #salva último registro com a data atual para que todas as linhas terminem no mesmo Y
